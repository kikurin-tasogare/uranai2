      ...
      <div class="result">
        ...
        <div class="result-block">
          ...
          ${(() => {
            ...
            const ris = Almanac.risshun(now.y);
            const gy = new Date() < ris ? now.y - 1 : now.y;
            const ys = ((gy - 4) % 10 + 10) % 10, yb = ((gy - 4) % 12 + 12) % 12;
            const t = tsuhen(ms.dayStem, ys);
            return `<p>今年(${gy}年・<b style="color:var(--gold-bright)">${STEMS[ys]}${BRANCHES[yb]}</b>の年)は、あなたの日主から見て<b style="color:var(--shu-bright)">「${t}」</b>が巡る年。${TSUHEN[t]}この気配を意識して一年の舵を取ると、流れに乗りやすくなります。</p>`;
          })()}
        </div>

        ${Uranai.glossary([
          ["日主(にっしゅ)", "日柱の上の文字(天干)。命式全体の主役で、「あなた自身」を表します。他のすべての星は、日主との関係で読まれます。"],
          ["命式(めいしき)", "生まれた年・月・日・時の四つの柱に干支を並べた表。あなたの運命の設計図にあたります。"],
          ["干支(かんし)", "十干(甲乙丙丁…)と十二支(子丑寅卯…)の組み合わせ60種。暦の一年・一月・一日・一刻それぞれに割り当てられています。"],
          ["通変星(つうへんせい)", "日主と他の柱の干との五行関係から導く10種の星(比肩・食神・正財・正官・印綬など)。才能や対人関係の質を表します。"],
          ["五行(ごぎょう)", "木・火・土・金・水。東洋思想で万物を構成する五つの気。命式の中の偏りが、その人の持ち味と課題を示します。"],
          ["節入り(せついり)", "二十四節気による月の切り替わりの瞬間。四柱推命の「月」はカレンダーの1日ではなく、立春・啓蟄などの節気で替わります。"],
          ["真太陽時(しんたいようじ)", "その土地で実際に太陽が南中する時刻を基準にした時間。日本標準時とは出生地の経度によって数十分ずれるため、時柱の判定で補正します。"],
          ["大運(たいうん)", "10年ごとに巡ってくる運気の流れ。月柱を起点に、生年の干の陰陽と性別で進む向き(順行・逆行)が決まります。"],
          ["立運(りつうん)", "大運が始まる年齢。生まれてから節入りまでの日数を3で割って求めます。"],
          ["年運(ねんうん)", "その年の干支が自分の日主に対して持つ意味。大運が10年の季節なら、年運は一年の天気です。"],
        ])}
      </div>
    `;
